      ...
      try {
        ...
        while (hasMoreData) {
          const response = await fetch(
            `/api/gamba/settled-games?page=${currentPage}&itemsPerPage=200&creator=${CREATOR_ADDRESS}&user=${userPublicKey}`,
          );
          if (!response.ok)
        ...
        <Tooltip
          content={({ active, payload }) => {
            if (active && payload && payload.length) {
              return (
                <div className="bg-muted p-4 rounded-lg text-white font-sans space-y-1">
